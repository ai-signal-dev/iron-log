...
export default function PresetsPage() {
  ...
  const createPreset = async () => {
    ...
    await db.presets.add({ name: newName.trim(), exercises: newExercises })
    setNewName('')
    setNewExercises([])
    setShowCreate(false)
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="p-3 bg-surface-hover rounded-full hover:bg-white/10"
        >
      ...
      {showCreate && (
        <div className="card p-4 space-y-4">
          <input
            ...
            placeholder="Preset name (e.g. Chest Day)"
          ...
          <div className="space-y-2">
            {newExercises.map((e, i) => (
              <div key={i} className="flex items-center justify-between bg-surface-hover rounded-lg px-3 py-2">
                ...
                <button
                  onClick={() => setNewExercises(prev => prev.filter((_, idx) => idx !== i))}
          ...
          <button onClick={createPreset} className="btn-primary w-full">
            Save Preset
          </button>
        </div>
      )}
      ...
      {presets?.length === 0 && !showCreate && (
        <div className="card p-8 text-center">
          <p className="text-text-secondary">No presets created yet</p>
        </div>
      )}
        ...
        <div key={preset.id} className="card p-4">
          ...
          <div className="space-y-1">
            {preset.exercises.map((e, i) => (
              <div key={i} className="text-sm text-text-secondary py-1 border-b border-white/5 last:border-0">
                {e.exerciseName}
              </div>
            ))}
          </div>
